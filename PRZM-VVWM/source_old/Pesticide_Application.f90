...
SUBROUTINE PESTAP(application_index)
      ...
      use  constants_and_Variables, ONLY: echofileunit, COVER,pest_app_method,  &
                                    delx,theta_zero,DEPI,TAPP,appeff, &
                                    conc_total_per_water,mass_in_compartment, &
                                    SOILAP,FOLPST,plant_app,Tband_top, ncom2, EchoFileUnit
      use utilities_1
      implicit none
    ...
    !K is the chenmical ID Number
      ...
      case (3) !Uniform  Incorporated pesticide application method 1
        plant_app= 0.
...
!       Solve for base of isosceles triangle given:length of 1 leg and area = 1.0
      ...
      case (100)  
          !this option is not accessible from pwc interface, it is here if you want to explore the default assumption that canopy spay that hits the ground 
...
!******************************************************************************************************************************
    SUBROUTINE pesticide_decreasing_distribution(APPAMT,DMAX,BASE,SLOPE,applied_to_soil )
      use  constants_and_Variables, ONLY: EchoFileUnit, delx, ncom2
      implicit none
