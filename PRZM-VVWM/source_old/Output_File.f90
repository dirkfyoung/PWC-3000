Module Output_File
  ...
  subroutine write_outputfile_header
      ...
      WRITE(TimeSeriesUnit,'(A48,1X,F7.3)')'United States of America, EPA PRZM5-VVWM Version', Version_Number  
  ...
  SUBROUTINE write_outputfile
    ...
    theta_end, CEVAP ,runoff_on_day,curve_number_daily,theta_sat,ainf,ncom2,snow,sedl,TDET,PVFLUX,DFFLUX,adflux,sdkflx,SUPFLX,UPFLUX,kd_new, &
    ...
    OUTPJJ,chem_id,NPLOTS, ARG,IARG2,PLNAME,MODE,DCOFLX,max_number_plots,Version_Number,julday1900,  &
     parent_runoff_series, parent_erosion_series, daughter_runoff_series,daughter_erosion_series,  &
    granddaughter_runoff_series,granddaughter_erosion_series,  runoff_series, erosion_series  ,day_number
...
!   Outputs user specified time series to time series plotting files
    ...
    runoff_series(day_number)  = runoff_on_day  !daily runoff in cm
    erosion_series(day_number)  = SEDL          ! Sediment flux (metric tons)
    
    
    parent_runoff_series(day_number) = ROFLUX(1)
    parent_erosion_series(day_number)  = ERFLUX(1) 
    
    if (nchem>1) then
       daughter_runoff_series(day_number)  = ROFLUX(2)
       daughter_erosion_series(day_number)  = ERFLUX(2) 
    end if
     
    if (nchem >2) then
       granddaughter_runoff_series(day_number)  = ROFLUX(3)
       granddaughter_erosion_series(day_number)  = ERFLUX(3) 
    end if
